...
def create_ampi_guide_db():
    ...
    db_guide = Chroma.from_documents(
        ...
        persist_directory=str(config.DB_AMPI_GUIDE_DIRECTORY)
    )
...
def carregar_documentos():
    """Carrega documentos da pasta base."""
    ...
    # Filtrar o guia AMPI para não processá-lo duas vezes
    documentos_filtrados = [
        doc for doc in documentos 
        if os.path.basename(doc.metadata.get('source', '')) != "Questionario_AMPI_Adaptado.pdf"
    ]
...
def vetorizar_chunks(chunks):
    """Vetoriza e salva chunks no ChromaDB."""
